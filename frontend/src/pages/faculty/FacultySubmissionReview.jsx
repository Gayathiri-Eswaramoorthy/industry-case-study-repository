...
function FacultySubmissionReview() {
  ...
  const canEvaluate = Boolean(submission?.canEvaluate);
  ...
  const handleSubmit = (e) => {
    ...
    if (hasCoMappings) {
      ...
      if (invalidRow) {
        setFormError("Enter valid score and max score for each course outcome.");
        return;
      }

      const coScores = coRows.map((row) => ({
        coId: Number(row.coId),
        score: Number(row.score),
        maxScore: Number(row.maxScore),
      }));

      const total = coRows.reduce(
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.35fr),minmax(0,1fr)]">
        ...
        <div className="space-y-6">
          {canEvaluate && isAlreadyEvaluated && (
            <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700 dark:border-emerald-500/30 dark:bg-emerald-950/30 dark:text-emerald-200">
              ...
              <p className="mt-0.5 text-xs">
                This submission has been evaluated. Contact an admin to override.
              </p>
            </div>
          )}
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            ...
            {canEvaluate && !isAlreadyEvaluated ? (
              ...
              <div className="space-y-3">
                {hasCoMappings && mappedOutcomes.length > 0 && isAlreadyEvaluated ? (
                  <div className="space-y-2">
                    {mappedOutcomes.map((row, index) => (
                      <div
                        key={row.coId}
                        className="flex items-center justify-between rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 dark:border-slate-700 dark:bg-slate-950"
                      >
                        <div>
                    ...
                    <div className="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-800 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100">
                      Total: {submission?.marksAwarded ?? totalScore} / {maxMarks}
                    </div>
                  </div>
                ) : (
                  <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-950">
                ...
                {isAlreadyEvaluated && submission?.facultyFeedback && (
                  <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-950">
                ...
                {!isAlreadyEvaluated && !canEvaluate && (
                  <div className="rounded-lg border border-dashed border-slate-300 bg-slate-50 px-4 py-6 text-center text-sm text-slate-500 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-400">
                    Not yet evaluated
                  </div>
                )}
                ...
                <Link
                  to="/faculty/submissions"
                  className="inline-flex items-center rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-100 dark:border-slate-600 dark:text-slate-200 dark:hover:bg-slate-800"
                >
                  Back to Queue
                </Link>
              </div>
