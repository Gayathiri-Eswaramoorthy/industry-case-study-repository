...
function PendingStudentsPage() {
  ...
  const pendingStudents = normalizeList(rawPendingStudents);
  ...
  const rejectMutation = useMutation({
    ...
    onError: (error) => {
      const message = error?.response?.data?.message || "Failed to reject student";
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
          Students waiting for your approval to join the platform
        </p>
      </div>
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        ...
        {!isLoading && isError ? (
          <div className="m-6 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 dark:border-red-500/40 dark:bg-red-950/30 dark:text-red-200">
            Failed to load pending student requests.
          </div>
        ) : null}

        {!isLoading && !isError && pendingStudents.length === 0 ? (
                ...
                {pendingStudents.map((student) => (
                  <tr key={student.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                    ...
                    <td className="px-4 py-3 text-slate-600 dark:text-slate-300">
                      {formatDate(student.createdAt || student.registeredAt)}
                    </td>
